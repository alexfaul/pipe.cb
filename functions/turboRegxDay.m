function affine_transforms = turboRegxDay(target_mov, varargin)
...
%   NOTE: hardcoded path to ImageJ.
    ...
    addOptional(p, 'xrun', true);  % Set to true if cross-run
...
%     addOptional(p, 'pre_register', false);  % Transformation register with DFT first
    addOptional(p, 'edges', [5, 5, 10, 10]);  % The edges to be removed
    ...
    if p.xrun
        ...
        % Within run
        data = imRead(path, tiff_start_vector(i), n, pmt, p.optolevel);
