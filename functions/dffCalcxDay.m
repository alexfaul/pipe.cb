function dffCalcxDay(root,percentile,xDayFactor,xDayFiles,time_window,runNums,dffTreatment,skipDFF)
if nargin<4, time_window=60; end
if nargin<5, runNums=[]; end                    % optional, will default to assuming 1:1 nidaq to registered suite2p data unless frame # mismatch
if nargin<6, dffTreatment=1;end
if nargin<7, skipDFF=0;end
...
suite2pData.xDayFiles=xDayFiles;
suite2pData.xDayFactor=xDayFactor;
...
%% Save struct
Filename=[newdir,'\',dsnidaq.mouse,'_',num2str(dsnidaq.date),'_',dsnidaq.run,'_','Suite2p_dffxDayAdjusted.mat'];
