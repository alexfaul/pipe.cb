...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
if nargin < 3 || isempty(typical_cell_size), typical_cell_size = 10; end
if nargin < 4 || isempty(max_distance), max_distance = 12; end

[microns_per_pixel,idxMag]=unique(microns_per_pixel);
if length(microns_per_pixel)>1
    sprintf('Magnifications do not match across days. Index position(s) %s are unique magnifications, drop unmatched mag days in bad_days and re-run',idxMag)
    return
end
...
[centroid_locations_corrected] = compute_centroid_locations(spatial_footprints_corrected, microns_per_pixel,typical_cell_size);
