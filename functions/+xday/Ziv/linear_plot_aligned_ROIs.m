...
% sub out the read tiff!!!!!!!
warped_imagesPath = ([obj.savedir '\FOV_registered_to_day_' num2str(obj.warptarget) '_final.tif']);
info=imfinfo(warped_imagesPath)
% warped_imagesPath=['Z:\AFdata\2p2019\Experiments\W10\xday_022222' '\FOV_registered_to_day_' num2str(obj.warptarget) '_final.tif']
%  info = imfinfo(['Z:\AFdata\2p2019\Experiments\W10\xday_022222' '\FOV_registered_to_day_' num2str(obj.warptarget) '_final.tif']);
for k = 1 : length(info)
    warped_images(:,:,k) = imread(warped_imagesPath, k);
end			
% get frame size
if ~isfield(obj.pars, 'sz')
    sz=[size(obj.masks_original,1), size(obj.masks_original,2)]  
else
    sz = obj.pars.sz;
end
