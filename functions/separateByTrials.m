function [dffTrials,baselineTrials, statsT,statsW]=separateByTrials(stimPath,timeWin)
...
for kk=1:length(orientationsUsed);
    ...
    dffTrials.(oris) = [];
    baselineTrials.(oris)=[]
    idx.(oris)=find(oriTrace==orientationsUsed(kk));
end
...
title([mouse,' ',runDate,' ',suite2pData.nidaqAligned.run, 'Percentage of Responsive Cells - Sign Rank']);
...
ylim([0 100])
ylabel('Number of Responsive cells - % of total')    
title([mouse,' ',runDate,' ',suite2pData.nidaqAligned.run, 'Percentage of Responsive Cells - T-test']);
%% 
dffTrials.runDate=runDate;
dffTrials.mouse=mouse;
