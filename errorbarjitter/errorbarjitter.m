...
%
%   errorbarjitter(data)
%   errorbarjitter(data,h,param1,val1,param2,val2,...)
%
%   e.g.
%   errorbarjitter(data,1,'barends','yes','colors',color_array)
%
%
...
%
% COLHEADERS: A cell array containing user defined column headers.
...
%
% STD: An array containing the standard deviations (or any other estimates
%   of distribution) for each datum in d, and an error bar equal to ± these
%   deviations will be plotted on each datum
%
% COLORS: Cell array equal in length to the number of data columns
%   containing user defined colors, using standard Matlab nomenclature
%
...
%
% COLOR_MAP: A matrix equal in dimension to the data matrix to plot
%   each datum a color from the scale of the current colormap
%
% DATA_MARKER: Matlab Marker specifier for data points ('o' default)
%
% DATA_MARKER_SIZE: Size of markers used for data points
%
% AVE_MARKER: Matlab Marker specifier for average points ('o' default)
%
% AVE_MARKER_SIZE: Size of markers used for average points
%
% DELETE_OUTLIERS: Two element array containing alpha and rep: e.g. [.05 1]
%   or 'No' (Default). Delete outliers using Grubb's test, as implemented
%   by Brett Shoelson
%   (www.mathworks.com/matlabcentral/fileexchange/3961-deleteoutliers)
%
...
% 2014/5/7      Small bug fix when no use of color_map
% 2014/5/8      Improved instructions
% 2014/5/14     Now clears figure before replotting and correctly colors
%               bar ends when this option chosen
...
if ~strcmp(average,'none')
    if strcmp(barends,'no') == 1
        ...
        if color_opts(2) == 0
            if color_opts(3) == 0
                errorbar(mean_d,e,[ave_marker 'k'],'MarkerFaceColor','k','MarkerEdgeColor','k','XData',x+offset);
            else
                for i = 1:n_categories
                    errorbar(mean_d(i),e(i),[ave_marker 'k'],'MarkerFaceColor','k','MarkerEdgeColor','k','Color',colors{i},'XData',x(i)+offset);
                end
            end
        else
            if color_opts(3) == 1
